...
pode ser previamente definido no código;*/
    ...
    val numInfo: Int = 21 //Número que eu quero encontrar na sequencia
    if (belongsFibonacci(numInfo)){
        println("Número $numInfo não pertence a sequencia de Fibonacci\n\n")
    } else {
        println("Número $numInfo pertence a sequncia de Fibonacci\n\n")
    }
...
pode ser previamente definida no código;*/
    ...
    val str: String = "Geovane"
    val stringCounted: Int = accountString(str)
    if (stringCounted > 0) {
        println("A letra 'a' existe na string e aparece ${stringCounted} vezes.\n\n")
    } else {
        println("A letra 'a' não existe na string.\n\n")
    }
...
Ao final do processamento, qual será o valor da variável SOMA?*/
    ...
    println("valor final da soma é: $soma\n\n")
...
//função que verifica se o número fornecido pertence a sequencia de fibonacci
private fun belongsFibonacci(numBelongs: Int): Boolean{

    var numPrevious: Int  = 0
    var numCurrent: Int = 1

    //Variavel boolean a ser retornada para saber se o número foi encontrado ou não
    var numFound: Boolean = false

    println("Sequencia de Fibonacci: ")
    println("$numPrevious")

    //verifica se o número desejado 'numBelongs' esta na sequencia de fibonacci, nesse exemplo 21
    while (numCurrent <= numBelongs){
        println("$numCurrent")
        if (numCurrent == numBelongs)
            numFound = true
...
private fun accountString(str: String): Int{

    // Verifica a quantidade de 'a' (minúsculo e maiúsculo)
    val amount = str.count { it == 'a' || it == 'A' }
